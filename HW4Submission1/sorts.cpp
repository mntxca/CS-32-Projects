...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for (vector<Sensor>::iterator iter = s.begin(); iter != s.end(); iter++)
    {
        Sensor nextItem = *iter;
        vector<Sensor>::iterator loc = iter;
        while (loc != s.begin() && comp(nextItem, *(loc - 1)))
        {
            *loc = *(loc-1);
            loc--;
        }
...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    vector<Sensor*> vp;
    /*for (vector<Sensor>::iterator iter = auxSensors.begin(); iter != auxSensors.end(); iter++)
    {
        vp.push_back(&(*iter));
    }*/
    for (int i = 0; i < auxSensors.size(); i++)
        vp.push_back(&auxSensors[i]);
    ...
    //        order.
    /*vector<Sensor*>::iterator itervp = vp.begin();
    for (vector<Sensor>::iterator iter = sensors.begin(); (iter != sensors.end()) && (itervp != vp.end()); iter++)
    {
        *iter = *(*itervp);
        itervp++;
    }*/
