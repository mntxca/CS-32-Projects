...
City::City(int nRows, int nCols)
    : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0), m_hist(nRows, nCols)
{
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    //keeps track of if there are any failures when preaching, if there are, call record() through m_hist object
    bool preachFail = false;
    ...
    if (preachFail)
        m_hist.record(m_player->row(), m_player->col());
}
